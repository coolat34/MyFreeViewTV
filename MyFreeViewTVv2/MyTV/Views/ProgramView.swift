...
//MARK: print values below
    ...
    struct DetailView: View {  /// Displays a rectangle when a program is tapped showing Season, Episode and Part
        let program: SavedProgram
        @Environment(\.dismiss) var dismiss
        @EnvironmentObject var HA: Handler
        var body: some View {
    ...
    var body: some View {
        let startTime = Int(HA.startHour)
            HStack(spacing: 0) {
                ...
                Text(String(format: "%02d:00", bannerLine[i]))
